...
test("kullanıcı doğru ad ve soyad girdiğinde ama email girmediğinde BİR hata mesajı render ediliyor.", async () => {
  render(<IletisimFormu />);
  const kullaniciAdi = screen.getAllByPlaceholderText(/Egemen/i);
  userEvent.type(kullaniciAdi, "Burcu");

  const kullaniciSoyadi = screen.getAllByPlaceholderText(/Sucuoglu/i);
...
test('geçersiz bir mail girildiğinde "email geçerli bir email adresi olmalıdır." hata mesajı render ediliyor', async () => {
  render(<IletisimFormu />);
  const kullaniciEmail = screen.getByPlaceholderText(/workintech@hotmail.com/i);
...
test('soyad girilmeden gönderilirse "soyad gereklidir." mesajı render ediliyor', async () => {
  render(<IletisimFormu />);
  const kullaniciSoyadi = screen.getByPlaceholderText(/Sucuoglu/i);
...
test("ad,soyad, email render ediliyor. mesaj bölümü doldurulmadığında hata mesajı render edilmiyor.", async () => {
  ...
  const kullaniciAdi = screen.getByPlaceholderText(/Egemen/i);
  userEvent.type(kullaniciAdi, "egemen");

  const kullaniciSoyadi = screen.getByPlaceholderText(/Sucuoglu/i);
  userEvent.type(kullaniciSoyadi, "sucuoglu");

  const kullaniciEmail = screen.getByPlaceholderText(/workintech@hotmail.com/i);
  userEvent.type(kullaniciEmail, "egemensucu@gmail.com");
  const submitButton = screen.getByTestId("submit-button");
  userEvent.click(submitButton);
  await waitFor(() => {
